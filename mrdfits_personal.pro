...
compile_opt idl2, hidden
  ...
  if (bitpix eq 16 && zero eq 32768L) ||                   $
     (bitpix eq 32 && zero eq ulong('2147483648')) ||        $
     (bitpix eq 64 && zero eq ulong64('9223372036854775808')) then begin
      return, 1
  endif
    endif
...
; NEW WAY: E.S.S.
    ...
    if N_elements(rows) EQ 0 then nrowp  = nrow else begin 
          bad = where((rows LT range[0]) || (rows GT range[1]), Nbad)
          if Nbad GT 0 then begin 
             print,'MRDFITS: Row numbers must be between 0 and ' + $
                    strtrim(nrow-1,2)      
             status = -1
...
;    rsize = fxpar(header, 'NAXIS1') 
  ...
  next_col: 
    ...
    if scaling then begin 
        w = where(scales ne 1.0d0 || offsets ne 0.0d0, Nw)
        scaling = Nw GT 0
    endif
